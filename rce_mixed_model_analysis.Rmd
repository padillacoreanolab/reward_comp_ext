...
for (col in all_columns) {
  ...
  # Fit the linear mixed-effects model
  model_data <- factored_lfp_trial_spectral |> drop_na(!!sym(col)) # Drop NA for the specific column
  ...
  predicted_df <- data.frame(subject_id = subject_values, original_value = original_values, predicted_value = predicted_values, col_name = col, trial_and_competitiveness_label=label_values, cluster_mean_trial_subject_thorax_velocity=velocity_values)
...
```{r}
# factored_lfp_trial_spectral[["current_subject"]]
predicted_values
```


```{r}
data.frame(factored_lfp_trial_spectral[["current_subject"]], factored_lfp_trial_spectral[[col]], predicted_values)
```


```{r}
colnames(predicted_df) <- c("subject_id", "original_value", "predicted_value")
  write.csv(predicted_df, file = predicted_file)
